...
Deno.serve(async (req) => {
  ...
  try {
    ...
    //
    // Les deux tables sont interrogées par des requêtes écrites en toutes
    // lettres, et non par un nom de table composé : `from()` et `select()`
    // tirent leur typage de littéraux, et les leur retirer fait échouer
    // `deno check`. La règle d'accès, elle, reste dans les policies —
    // `lecons_select_gated` et `ressources_select_gated` exigent l'une comme
    // l'autre une étape déverrouillée, la seconde y ajoutant une ressource
    // active.
    let dureeVideo: number | null = null;
    let aUneVideo = false;
    let ligneTrouvee = false;

    if (id_lecon) {
      const { data } = await porteur
        .from('lecons')
        .select('duree_s, video_hebergee')
        .eq('id_lecon', id_lecon)
        .maybeSingle();
      ligneTrouvee = data !== null;
      dureeVideo = data?.duree_s ?? null;
      aUneVideo = data?.video_hebergee === true;
    } else {
      const { data } = await porteur
        .from('ressources')
        .select('a_video_hebergee')
        .eq('id_ressource', id_ressource as string)
        .maybeSingle();
      ligneTrouvee = data !== null;
      aUneVideo = data?.a_video_hebergee === true;
    }
    ...
    const { data: source } = id_lecon
      ? await admin.from('lecons').select('video_url').eq('id_lecon', id_lecon).maybeSingle()
      : await admin
          .from('ressources')
          .select('video_url')
          .eq('id_ressource', id_ressource as string)
    ...
    if (!cle) {
      return json(req, { url: source.video_url, signee: false, expire_le: null }, 200);
    }
    ...
    // minutes.
    const duree = dureeJeton(dureeVideo);
    const url = await signerUrlVideo(source.video_url, cle, duree);
